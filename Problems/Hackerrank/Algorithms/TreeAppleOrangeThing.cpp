...
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    vector<int> appleposition;
    vector<int> orangeposition;
 
    for(int i=0;i<apples.size();i++){
        appleposition.push_back((apples.at(i))+a);
    }

    for(int i=0;i<oranges.size();i++){
        orangeposition.push_back((oranges.at(i)+b));
    }

    int applehouse=0;
    for(int i=0;i<apples.size();i++){
        if((appleposition.at(i)>=s)&(appleposition.at(i)<=t)){
            applehouse++;
        }
    }
    ...
    int orangehouse=0;
    for(int i=0;i<oranges.size();i++){
        if((orangeposition.at(i)>=s)&(orangeposition.at(i)<=t)){
            orangehouse++;
        }
    }

    cout<<applehouse<<endl;
    cout<<orangehouse<<endl;
